...
const renderProfile = (user) => {
    ...
    website.setAttribute('href', user.website !== null ? user.website : website.removeAttribute('href'))
    email.setAttribute('href', user.email !== null ? user.email : email.removeAttribute('href'))
    twitter.setAttribute('href', user.twitter !== null ? 'https://twitter.com/' + user.twitter : twitter.removeAttribute('href'))
    ...
    for (let i = 0; i < user.repos.length; i++) {
        ...
        repoTag.href = user.repos[i].url
        repoTag.innerHTML = user.repos[i].name
...
const parseData = (user_data) => {
    ...
    parsedData = fetchPopularRepos(repos_url).then(popular_repos => {
        return {
            avatar: user_data.avatar_url !== null ? user_data.avatar_url : 'https://external-content.duckduckgo.com/iu/?u=https%3A%2F%2Fpngimg.com%2Fuploads%2Fbokeh%2Fbokeh_PNG27.png',
            bio: bio,
            name: name === null ? login : name,
            website: blog === null || blog === undefined ? '#' : blog,
...
const setImagePalette = (img_url) => {
    ...
    img.addEventListener('load', () => {
        ...
        document.documentElement.style.setProperty('--primary', rgbToHex(...palette[2]))
        document.documentElement.style.setProperty('--secondary', rgbToHex(...palette[4]))
        document.documentElement.style.setProperty('--tertiary', rgbToHex(...palette[7]))
    })
